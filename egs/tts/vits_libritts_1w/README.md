...
# vits_libritts_1w Recipe

In this recipe, we will show how to train [vits_libritts_1w](https://arxiv.org/abs/2106.06103) using Amphion's infrastructure. vits_libritts_1w is an end-to-end TTS architecture that utilizes conditional variational autoencoder with adversarial learning.
...
```bash
sh egs/tts/vits_libritts_1w/run.sh --stage 1
```
...
```bash
sh egs/tts/vits_libritts_1w/run.sh --stage 2 --name [YourExptName]
```
...
```bash
sh egs/tts/vits_libritts_1w/run.sh --stage 3 --gpu "0" \
	--infer_expt_dir /mnt/workspace/wangmingxuan/vits_on_libritts_hifitts/logs/LibriTTS_1w \
	--infer_output_dir /mnt/workspace/wangmingxuan/vits_on_libritts_hifitts/logs/LibriTTS_1w/result \
	--infer_mode "batch" \
...
sh egs/tts/vits_libritts_1w/run.sh --stage 3 --gpu "0" --infer_expt_dir /mnt/workspace/wangmingxuan/vits_on_libritts_hifitts/logs/LibriTTS_1w --infer_output_dir /mnt/workspace/wangmingxuan/vits_on_libritts_hifitts/logs/LibriTTS_1w/result --infer_mode "single" --infer_text "This is a clip of generated speech with the given text from a TTS model."
```

We will release a pre-trained vits_libritts_1w model trained on LJSpeech. So you can download the pre-trained model and generate speech following the above inference instruction.
